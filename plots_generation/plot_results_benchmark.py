...
def plot_results(
    ...
    results_dirs = [
        experiment_path / x
        for x in os.listdir(experiment_path)
        ...
        try:
            ...
            if normalize:
                ...
                adapted_dataset_name = (
                    "/".join(dataset_parts[:3]) + "/" + "_".join(dataset_parts[3:])
                )
                ...
                temp_results = (
                    (best_performance - np.array(temp_results))
    ...
    plt.fill_between(
        common_indexes, results_avg - results_std, results_avg + results_std, alpha=0.2
    )
