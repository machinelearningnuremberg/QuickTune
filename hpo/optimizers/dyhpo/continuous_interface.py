...
if __name__ == "__main__":
    ...
    optimizer = prepare_dyhpo_optimizer(
        metadataset,
        name="dyhpo_non_metatrained1",
        hidden_dim=32,
        output_dim=32,
    ...
    max_normalized = (
        metadataset.args_max - metadataset.args_mean
    ) / metadataset.args_std
    min_normalized = (
        metadataset.args_min - metadataset.args_mean
    ) / metadataset.args_std
    ...
    extended_mean_predictions, extended_std_predictions, extedend_cost_predictions = (
        optimizer.model.predict_pipeline(train_data, extended_test_data)
    )

    # treating special hps
    idx_num_classes = np.where(
        np.array(metadataset.hyperparameter_names) == "num_classes"
    )[0]
    ...
    def objective(x):
        ...
        test_data = {"X_test": x, "test_budgets": budgets, "test_curves": curves}

        mean_predictions, std_predictions, costs = optimizer.model.predict_pipeline(
    ...
    suggested_x = (
        suggested_x * metadataset.args_std[metadataset.hyperparameter_names]
        + metadataset.args_mean[metadataset.hyperparameter_names]
    )
    aft_config = optimizer.to_aft_config(suggested_x)
    aft_config["opt_betas"] = (
        aft_config["opt_betas"].replace("[", "").replace("]", "").replace(", ", " ")
    )
    ...
    output = os.path.join(
        rootdir, "..", "..", "..", "experiments", "output", "temp", run_id, dataset
    )

    task_info, dataset = get_task_info(version=version, set=set, dataset=dataset)
    experiment = get_continuous_config_code(aft_config)
    perf, cost = eval_finetune_conf(
        aft_config, task_info, experiment=experiment, data_path=data_path, output=output
    )
