...
def run_quicktune(
    ...
    output_dir = os.path.join(
        rootdir, "..", "..", "..", "experiments", "output", "hpo", experiment_id
    )
    ...
    while not done:
        ...
        if perf > highest_perf:
            ...
            shutil.copy(
                os.path.join(output, experiment, "last.pth.tar"),
                os.path.join(output_dir, new_dataset_name, "best_model.pt"),
            )

        # save info dict
        with open(
            os.path.join(output_dir, new_dataset_name, "info_dict.json"), "w"
        ) as f:
            # dump json
        ...
        with open(
            os.path.join(output_dir, new_dataset_name, "perf_curves.json"), "w"
        ) as f:
            # dump json
...
if __name__ == "__main__":
    ...
    datasets_for_split = CostPredictorTrainer.get_splits(
        None,
        datasets,
    ...
    if args.dataset_name is None:
        ...
        metafeatures = prepare_metafeatures(
            dataset_size, num_classes, image_size, num_channels
        ).to(device)
        run_quicktune(
            args.dataset_name,
