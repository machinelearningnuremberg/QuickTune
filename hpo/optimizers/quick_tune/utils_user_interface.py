...
def get_optimizer(metadataset, budget_limit , new_dataset_name, dataset_name, temp_output_dir, output_dir,
    ...
    optimizer = create_qt_optimizer(metadataset, experiment_id = experiment_id,
...
def get_task_info(version, set, dataset, rootdir):
    ...
    with open(os.path.join(rootdir, "..", "AutoFinetune", DEFAULT_FOLDER, version, set, dataset, "info.json")) as f:
...
def run_quicktune(dataset_name, ss, metadataset, budget_limit, experiment_id, create_optimizer=False,
                               meta_train=False, cost_aware = False, split_id=0, verbose = False, task_info = None,
                                ...
                                rootdir = None, train_iter=10000, num_configurations = 400,
    ...
    if task_info is None:
        ...
        task_info, dataset = get_task_info(version=aft_set, set=set,
    ...
    if create_optimizer:
        ...
        optimizer = get_optimizer(metadataset, budget_limit, new_dataset_name,
                                  dataset_name, temp_output_dir, experiment_dir, experiment_id, log_indicator,
        ...
        if optimizer_name is None:
            ...
            optimizer_path = os.path.join(rootdir, "output",  optimizer_name, "optimizer.pkl")
    ...
    temp_output = os.path.join(rootdir, "output", experiment_id, "temp", run_id,
                          dataset)
    ...
    while(not done):
        ...
        perf, cost, status = eval_finetune_conf(aft_config, task_info, budget =  budget, experiment = temp_config_id,
                                                  data_path = data_path,
                                                  output = temp_output,
                                                  verbose=verbose,
        ...
        if perf > highest_perf:
            ...
            shutil.copy(os.path.join(temp_output, temp_config_id, "last.pth.tar"), 
                        os.path.join(experiment_dir, new_dataset_name, "best_model.pt"))

        #save info dict
        with open(os.path.join(experiment_dir, new_dataset_name, "info_dict.json"), "w") as f:
            #dump json
            json.dump(info_dict, f)

        #save curves in output folder
        with open(os.path.join(experiment_dir, new_dataset_name, "perf_curves.json"), "w") as f:
            #dump json
