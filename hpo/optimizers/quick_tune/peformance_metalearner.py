...
class PerformanceMetaLearner:
    ...
    def get_batch(self, mode="train", dataset=None):
        ...
        metafeatures = (
            torch.FloatTensor(metafeatures).to(self.device) / 1000
        )  # TODO: Proper scaling
    ...
    def meta_train(self):
        ...
        if self.with_scheduler:
            scheduler = torch.optim.lr_scheduler.CosineAnnealingLR(
                self.model.optimizer, self.train_iter, eta_min=1e-7
            )
        else:
            scheduler = None
        ...
        for i in range(self.train_iter):
            ...
            if i % self.test_freq == 0:
                ...
                if val_error < min_eval_error:
                    ...
                    self.model.save_checkpoint()
                    self.logger.info(
                        f"Saving checkpoint in meta-learning with error: {min_eval_error}"
                    )
    ...
    def meta_val(self):
        ...
        for i in range(self.test_iter):
            ...
            means, stds, costs = self.model.predict_pipeline(
                batch_train, batch_test, to_numpy=False
            )
