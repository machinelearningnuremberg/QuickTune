...
class QuickTuneMetaDataset:
    ...
    def aggregate_curves(self):
        ...
        for file in files:
            try:
                ...
                curves_data = pd.read_csv(
                    f"{self.path}/curves/{self.version}/{file}", nrows=52, index_col=0
                )

                if curves_data.shape[0] < 2:
                    if (curves_data["eval_top1"][0] == 0.0) or (
                        np.isnan(curves_data["eval_top1"][0])
                    ):
                        continue
                elif (
                    (len(curves_data) == 26)
                    and (
                        curves_data["eval_top1"][24] - curves_data["eval_top1"][25] > 20
                    )
                    and args["amp"]
                ):
                ...
                if isinstance(max_eval_top1, float) and isinstance(
                    max_eval_top5, float
                ):
                ...
                for name in self.curves_names:
                    aggregated_curves[name][dataset][run] = curves_data[
                        name
                    ].values.tolist()
        ...
        for name in self.curves_names:
            with open(
                f"{self.path}/qt_metadataset/{self.version}/{name}.json", "w"
            ) as outfile:
                json.dump(aggregated_curves[name], outfile)

        with open(
            f"{self.path}/qt_metadataset/{self.version}/args.json", "w"
        ) as outfile:
    ...
    def load_curves(self, curve_names):
        ...
        for name in curve_names:
            with open(
                f"{self.path}/qt_metadataset/{self.version}/{name}.json", "r"
            ) as stream:
    ...
    def load_metafeatures(self):
        ...
        # read json file
        with open(
            f"{self.path}/qt_metadataset/dataset-meta-features/{superset}/meta-features.json",
            "r",
        ) as stream:
        ...
        if superset == "meta-album":
            metafeatures_df.index = [
                "mtlbm/" + x for x in metafeatures["dataset_names"]
            ]
        else:
    ...
    def load_args(self, preprocess=False, file_name="unnormalized_args_table"):
        ...
        if not os.path.exists(
            f"{self.path}/qt_metadataset/{self.version}/{file_name}.csv"
        ):
            with open(
                f"{self.path}/qt_metadataset/{self.version}/args.json", "r"
            ) as stream:
        ...
        if self.standardize_numerical_args:
            ...
            for col in self.args_df.columns:
                if (
                    ...
                    self.args_df[col] = (
                        self.args_df[col] - self.args_mean[col]
                    ) / self.args_std[col]
    ...
    def preprocess_args(self, args_df):
        ...
        if self.encode_categorical_args:
            ...
            df_cat_transformed = pd.DataFrame(
                df_cat_transformed, columns=new_cat_columns
            )
        else:
    ...
    def preload_hyperparameters_candidates(self):
        ...
        for arg in self.dataset_args_df.columns:
            if (
                arg.startswith("cat__dataset")
                or arg.startswith("cat__project")
                or (arg in args_to_remove)
            ):
                args.remove(arg)
    ...
    def get_runs(self, dataset_name=None, augmentation_id=None):
        ...
        if augmentation_id is not None:
            adapted_dataset_name = "_".join(
                dataset_name.split("/")[1:] + [str(augmentation_id), ""]
            )
    ...
    def set_dataset_name(self, dataset_name, augmentation_id=None):
        ...
        if augmentation_id is not None:
            self.dataset_args_df = self.dataset_args_df.loc[
                self.args_df["augmentation_id"] == augmentation_id
            ].copy()
            self.dataset_args_df = self.dataset_args_df.drop(
        ...
        if self.target_model is not None:
            if self.action_on_model == "omit_it":  # omit one
                ...
                self.dataset_args_df = self.dataset_args_df[
                    self.dataset_args_df[ohe_target_model_name] != 1
                ].copy()
    ...
    def get_incumbent_curve(self):
        ...
        ), "Dataset name not set. Use set_dataset_name() to set it."
    ...
    def set_subsample_models(self, subsample_models_in_hub):
        self.subsample_models_in_hub = subsample_models_in_hub
        self.model_vars = self.dataset_args_df.columns[
            self.dataset_args_df.columns.str.startswith("cat__model_")
        ]
        self.selected_models = np.random.choice(
            self.model_vars, self.subsample_models_in_hub, replace=False
        )
