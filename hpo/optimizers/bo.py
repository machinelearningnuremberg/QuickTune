def BO(
    ...
    while current_budget < budget_limit:
        ...
        cost_curve_eval = metadataset.get_curve(
            hp_index, budget, curve_name="eval_time"
        )
        cost_curve_train = metadataset.get_curve(
            hp_index, budget, curve_name="train_time"
        )
        ...
        if observe_cost:
            ...
            observed_cost = None
        overhead_time = optimizer.observe(
            hp_index, budget, performance_curve, observed_cost
        )
        ...
        optimizer_budget.extend(
            [
                i
                for i in range(
                    optimizer_budget[-1] + 1,
                    optimizer_budget[-1] + budget_increment + 1,
                )
            ]
        )
        ...
        optimizer_performance.extend(
            performance_curve[budget - budget_increment : budget]
        )
