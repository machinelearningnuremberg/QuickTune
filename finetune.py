...
group.add_argument('--st_checkpoint_dir', type=str, default=None, help='path to the checkpoint directory of the model'
                                                                       'when using synetune')
...
def main(args, args_text):
    ...
    if args.dataset_augmentation_path:
        try:
            ...
            assert args.dataset.startswith("tfdsicgn"), "dataset_augmentation_path is only supported for tfds_icgn datasets"
            ...
            print(f"Number of train samples: {icgen_dataset_info['number_train_samples_per_class']*actual_num_classes}")
    ...
    if compute_relationship:
        ...
        output_dir = utils.get_outdir(os.path.join(args.output, "..", "relationships") if args.output else './output/relationships')
    ...
    try:
        for epoch in range(start_epoch, num_epochs):
            ...
            if args.linear_probing and (epoch == (num_epochs-start_epoch)//2):
                model, head_name = prepare_model_for_finetuning(model, pct_to_freeze=args.pct_to_freeze,
                                                                return_features=return_features,
                                                                return_source_output=return_source_output,
                                                                change_head=False)
                #if device_count>1: model = nn.DataParallel(model).to(device)
...
def train_one_epoch(
    ...
    for batch_idx, (input, target) in enumerate(loader):
        ...
        if last_batch or batch_idx % args.log_interval == 0:
            ...
            backbone_grad_norm = (num_logs-1)*backbone_grad_norm/num_logs + (1/num_logs)*temp_backbone_grad_norm
    ...
    return OrderedDict([('loss', losses_m.avg), ('head_grad_norm', head_grad_norm), ('backbone_grad_norm', backbone_grad_norm)])
