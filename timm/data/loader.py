...
def fast_collate(batch):
    ...
    #    targets = torch.tensor([b[1] for b in batch], dtype=torch.int64)
    #    assert len(targets) == batch_size
    #    shape = torch.from_numpy(np.asarray(batch[0][0])).transpose(0,2).shape
    #    tensor = torch.zeros((batch_size, *shape), dtype=torch.uint8)
    #    for i in range(batch_size):
    #        tensor[i].copy_(torch.from_numpy(np.asarray(batch[i][0])).transpose(0,2))
    #    return tensor, targets
    else:
        assert False
...
def create_loader(
    ...
    loader_args = dict(
        ...
        drop_last=drop_last and is_training,
